...
async def callback_run_quest(callback: types.CallbackQuery):
    ...
    user_data = await user_data_rep.get_one(tg_chat_id=callback.from_user.id)
...
async def run_quest(session, user_data: dict, new_step=settings.START_TITLE):
    user_data_rep = UserDataRepository(session)
    ...
    if new_step == settings.STOP_TITLE:
        if user_data['is_in_quest']:
            await user_data_rep.update(uid=user_data['uid'], new_is_in_quest=False)
            user_data['is_in_quest'] = False
            user_data['step'] = new_step

    elif user_data['step'] != new_step:
        await user_data_rep.update(uid=user_data['uid'], new_step=new_step)
        user_data['step'] = new_step

    if user_data['step'] != settings.STOP_TITLE:
        if not user_data['is_in_quest']:
            await user_data_rep.update(uid=user_data['uid'], new_is_in_quest=True)
            user_data['is_in_quest'] = True

    message = await dp.bot.send_message(
        chat_id=user_data['tg_chat_id'],
        text=(await ScriptRepository(session).get_one(title=user_data['step']))['text'],
        reply_markup=await get_markup(session, user_data['step'], user_data)
    )

    await user_data_rep.update(uid=user_data['uid'], new_quest_message_id=message.message_id)
